...
public class CourseDAO implements CourseRepository{
	...
	public List<Object> findMyGrades(int id){
		...
		Session session = sessionFactory.openSession();
		Query qry= session.createQuery("select t.name, t.courseName, c.examDate, c.grade from Teacher t, Course c, Student s "
                + " where t.idteacher = c.idteacher and s.idstudent=c.idstudent and s.idstudent= :idstudent");
		
        List l = qry.setParameter("idstudent",id).list();
		session.close();
		return l;
	}
